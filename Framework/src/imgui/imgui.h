...
IMGUI_API void EndFrame(); // ends the ImGui frame. automatically called by Render(), you likely don't need to call that
                           // yourself directly. If you don't need to render data (skipping rendering) you may call
                           // EndFrame() but you'll have wasted CPU already! If you don't need to render, better to not
                           // create any imgui windows and not call NewFrame() at all!
...
IMGUI_API void ShowDemoWindow(
  bool* p_open = NULL); // create demo/test window (previously called ShowTestWindow). demonstrate most ImGui features.
                        // call this to learn about the library! try to make it always available in your application!
IMGUI_API void ShowMetricsWindow(
  bool* p_open = NULL); // create metrics window. display ImGui internals: draw commands (with individual draw calls and
                        // vertices), window list, basic internal state, etc.
IMGUI_API void ShowStyleEditor(
  ImGuiStyle* ref = NULL); // add style editor block (not a window). you can pass in a reference ImGuiStyle structure to
...
// Windows
// (Begin = push window to the stack and start appending to it. End = pop window from the stack. You may append multiple
// times to the same window during the same frame) Begin()/BeginChild() return false to indicate the window being
// collapsed or fully clipped, so you may early out and omit submitting anything to the window. You need to always call
// a matching End()/EndChild() for a Begin()/BeginChild() call, regardless of its return value (this is due to legacy
// reason and is inconsistent with BeginMenu/EndMenu, BeginPopup/EndPopup and other functions where the End call should
// only be called if the corresponding Begin function returned true.) Passing 'bool* p_open != NULL' shows a close
// widget in the upper-right corner of the window, which when clicking will set the boolean to false. Use child windows
// to introduce independent scrolling/clipping regions within a host window. Child windows can embed their own child.
IMGUI_API bool Begin(const char* name, bool* p_open = NULL, ImGuiWindowFlags flags = 0);
IMGUI_API void End();
IMGUI_API bool BeginChild(
  const char* str_id, const ImVec2& size = ImVec2(0, 0), bool border = false,
  ImGuiWindowFlags flags =
    0); // Begin a scrolling region. size==0.0f: use remaining window size, size<0.0f: use remaining window size minus
...
IMGUI_API bool IsWindowHovered(
  ImGuiHoveredFlags flags =
    0); // is current window hovered (and typically: not blocked by a popup/modal)? see flags for options. NB: If you
        // are trying to check whether your mouse should be dispatched to imgui or to your app, you should use the
...
IMGUI_API void SetNextWindowContentSize(
  const ImVec2&
    size); // set next window content size (~ enforce the range of scrollbars). not including window decorations (title
           // bar, menu bar, etc.). set an axis to 0.0f to leave it automatic. call before Begin()
...
IMGUI_API void SetScrollHere(
  float center_y_ratio = 0.5f); // adjust scrolling amount to make current cursor position visible. center_y_ratio=0.0:
                                // top, 0.5: center, 1.0: bottom. When using to make a "default/current item" visible,
                                // consider using SetItemDefaultFocus() instead.
IMGUI_API void SetScrollFromPosY(
  float pos_y, float center_y_ratio = 0.5f); // adjust scrolling amount to make given position valid. use GetCursorPos()
                                             // or GetCursorStartPos()+offset to get valid positions.
...
IMGUI_API void PushItemWidth(
  float item_width); // width of items for the common item+label case, pixels. 0.0f = default to ~2/3 of windows width,
                     // >0.0f: width in pixels, <0.0f align xx pixels to the right of window (so -1.0f always align
                     // width to the right side)
IMGUI_API void PopItemWidth();
IMGUI_API float CalcItemWidth(); // width of item given pushed settings and current cursor position
IMGUI_API void PushTextWrapPos(
  float wrap_pos_x = 0.0f); // word-wrapping for Text*() commands. < 0.0f: no wrapping; 0.0f: wrap to end of window (or
...
IMGUI_API void PushButtonRepeat(
  bool repeat); // in 'repeat' mode, Button*() functions return repeated true in a typematic manner (using
                // io.KeyRepeatDelay/io.KeyRepeatRate setting). Note that you can call IsItemActive() after any Button()
...
IMGUI_API void TextWrapped(const char* fmt, ...)
  IM_FMTARGS(1); // shortcut for PushTextWrapPos(0.0f); Text(fmt, ...); PopTextWrapPos();. Note that this won't work on
                 // an auto-resizing window if there's no other widgets to extend the window width, yoy may need to set
                 // a size using SetNextWindowSize().
...
IMGUI_API void SetColorEditOptions(
  ImGuiColorEditFlags flags); // initialize current options (generally on application startup) if you want to select a
                              // default format, picker type, etc. User will be able to change many settings, unless you
                              // pass the _NoOptions flag to your calls.
...
IMGUI_API bool TreeNode(const char* str_id, const char* fmt, ...)
  IM_FMTARGS(2);                                                                 // helper variation to completely decorelate the id from the displayed string. Read the FAQ about why
                                                                                 // and how to use ID. to align arbitrary text at the same level as a TreeNode() you can use Bullet().
...
IMGUI_API bool Selectable(
  ...
  const ImVec2& size =
    ImVec2(0, 0)); // "bool selected" carry the selection state (read-only). Selectable() is clicked is returns true so
...
IMGUI_API bool ListBoxHeader(
  const char* label, const ImVec2& size = ImVec2(
                       0, 0));                                                              // use if you want to reimplement ListBox() will custom data or interactions. if the
...
IMGUI_API bool MenuItem(const char* label, const char* shortcut = NULL, bool selected = false,
                        bool enabled = true); // return true when activated. shortcuts are displayed for convenience but
                                              // not processed by ImGui at the moment
...
IMGUI_API void SetTooltip(const char* fmt, ...) IM_FMTARGS(
  1); // set a text-only tooltip, typically use with ImGui::IsItemHovered(). overidde any previous call to SetTooltip().
IMGUI_API void SetTooltipV(const char* fmt, va_list args) IM_FMTLIST(1);

// Popups
IMGUI_API void OpenPopup(
  const char* str_id); // call to mark popup as open (don't call every frame!). popups are closed when user click
                       // outside, or if CloseCurrentPopup() is called within a BeginPopup()/EndPopup() block. By
                       // default, Selectable()/MenuItem() are calling CloseCurrentPopup(). Popup identifiers are
                       // relative to the current ID-stack (so OpenPopup and BeginPopup needs to be at the same level).
...
IMGUI_API bool BeginPopupContextItem(
  ...
  int mouse_button = 1); // helper to open and begin popup when clicked on last item. if you can pass a NULL str_id only
                         // if the previous item had an id. If you want to use that on a non-interactive item such as
...
IMGUI_API bool OpenPopupOnItemClick(
  const char* str_id = NULL,
  int mouse_button = 1);                        // helper to open popup when clicked on last item. return true when just opened.
...
// Columns
// You can also use SameLine(pos_x) for simplified columns. The columns API is still work-in-progress and rather
// lacking.
...
IMGUI_API float GetColumnOffset(
  int column_index = -1); // get position of column line (in pixels, from the left side of the contents region). pass -1
                          // to use current column, otherwise 0..GetColumnsCount() inclusive. column 0 is typically 0.0f
...
IMGUI_API bool SetDragDropPayload(
  ...
  ImGuiCond cond = 0);                // type is a user defined string of maximum 32 characters. Strings starting with '_' are reserved
                                      // for dear imgui internal types. Data is copied and held by imgui.
IMGUI_API void EndDragDropSource();   // only call EndDragDropSource() if BeginDragDropSource() returns true!
IMGUI_API bool BeginDragDropTarget(); // call after submitting an item that may receive an item. If this returns true,
...
// Focus, Activation
// (Prefer using "SetItemDefaultFocus()" over "if (IsWindowAppearing()) SetScrollHere()" when applicable, to make your
// code more forward compatible when navigation branch is merged)
IMGUI_API void SetItemDefaultFocus(); // make last item the default focused item of a window. Please use instead of "if
                                      // (IsWindowAppearing()) SetScrollHere()" to signify "default item".
...
IMGUI_API bool IsKeyDown(
  int user_key_index); // is key being held. == io.KeysDown[user_key_index]. note that imgui doesn't know the semantic
                       // of each entry of io.KeysDown[]. Use your own indices/enums according to how your
...
IMGUI_API ImVec2 GetMouseDragDelta(
  int button = 0, float lock_threshold =
...
IMGUI_API void CaptureKeyboardFromApp(
  bool capture = true);                                  // manually override io.WantCaptureKeyboard flag next frame (said flag is entirely left for your
...
enum ImGuiWindowFlags_ {
  ...
  ImGuiWindowFlags_NoScrollWithMouse =
    1 << 4,                                   // Disable user vertically scrolling with mouse wheel. On child window, mouse wheel will be forwarded to the
  ...
  ImGuiWindowFlags_NoBringToFrontOnFocus = 1 << 13,   // Disable bringing window to front when taking focus (e.g. clicking
                                                      // on it or programatically giving it focus)
  ImGuiWindowFlags_AlwaysVerticalScrollbar = 1 << 14, // Always show vertical scrollbar (even if ContentSize.y < Size.y)
  ImGuiWindowFlags_AlwaysHorizontalScrollbar =
    1 << 15, // Always show horizontal scrollbar (even if ContentSize.x < Size.x)
  ImGuiWindowFlags_AlwaysUseWindowPadding =
    1 << 16,                              // Ensure child windows without border uses style.WindowPadding (ignored by default for non-bordered child
                                          // windows, because more convenient)
...
enum ImGuiInputTextFlags_ {
  ...
  ImGuiInputTextFlags_CallbackAlways =
    1 << 8, // Call user function every time. User code may query cursor position, modify text buffer.
  ImGuiInputTextFlags_CallbackCharFilter = 1
                                           << 9, // Call user function to filter character. Modify data->EventChar to
                                                 // replace/filter input, or return 1 in callback to discard character.
  ImGuiInputTextFlags_AllowTabInput = 1 << 10,   // Pressing TAB input a '\t' character into the text field
  ImGuiInputTextFlags_CtrlEnterForNewLine =
    1 << 11,                                        // In multi-line mode, unfocus with Enter, add new line with Ctrl+Enter (default is opposite: unfocus with
  ...
  ImGuiInputTextFlags_NoUndoRedo =
    1 << 16,                                     // Disable undo/redo. Note that input text owns the text data while active, if you want to provide your own
                                                 // undo/redo stack you need e.g. to call ClearActiveID().
  ImGuiInputTextFlags_CharsScientific = 1 << 17, // Allow 0123456789.+-*/eE (Scientific notation input)
  ImGuiInputTextFlags_CallbackResize =
    1 << 18, // Allow buffer capacity resize + notify when the string wants to be resized (for string types which hold a
...
enum ImGuiTreeNodeFlags_ {
  ...
  // ImGuiTreeNodeFlags_NoScrollOnOpen     = 1 << 12,  // FIXME: TODO: Disable automatic scroll on TreePop() if node got
...
enum ImGuiComboFlags_ {
  ...
  ImGuiComboFlags_HeightSmall = 1 << 1,    // Max ~4 items visible. Tip: If you want your combo popup to be a specific size
...
enum ImGuiDragDropFlags_ {
  ...
  ImGuiDragDropFlags_SourceNoDisableHover =
    1 << 1, // By default, when dragging we clear data so that IsItemHovered() will return false, to avoid subsequent
            // user code submitting tooltips. This flag disable this behavior so you can still call IsItemHovered() on
            // the source item.
  ...
  ImGuiDragDropFlags_SourceAllowNullID =
    1 << 3, // Allow items such as Text(), Image() that have no unique identifier to be used as drag source, by
            // manufacturing a temporary identifier based on their window-relative position. This is extremely unusual
  ...
  ImGuiDragDropFlags_SourceAutoExpirePayload =
    1 << 5, // Automatically expire the payload if the source cease to be submitted (otherwise payloads are persisting
            // while being dragged)
...
enum ImGuiNavInput_ {
  ...
  ImGuiNavInput_FocusPrev,   // next window (w/ PadMenu)                     // e.g. L1 or L2 (PS4), LB or LT (Xbox), L or
                             // ZL (Switch)
  ImGuiNavInput_FocusNext,   // prev window (w/ PadMenu)                     // e.g. R1 or R2 (PS4), RB or RT (Xbox), R or
                             // ZL (Switch)
  ImGuiNavInput_TweakSlow,   // slower tweaks                                // e.g. L1 or L2 (PS4), LB or LT (Xbox), L or
                             // ZL (Switch)
  ImGuiNavInput_TweakFast,   // faster tweaks                                // e.g. R1 or R2 (PS4), RB or RT (Xbox), R or
                             // ZL (Switch)
...
enum ImGuiConfigFlags_ {
  ...
  ImGuiConfigFlags_NavEnableSetMousePos =
    1 << 2, // Instruct navigation to move the mouse cursor. May be useful on TV/console systems where moving a virtual
            // mouse is awkward. Will update io.MousePos and set io.WantSetMousePos=true. If enabled you MUST honor
            // io.WantSetMousePos requests in your binding, otherwise ImGui will react as if the mouse is jumping around
            // back and forth.
  ...
  ImGuiConfigFlags_NoMouseCursorChange =
    1 << 5, // Instruct back-end to not alter mouse cursor shape and visibility. Use if the back-end cursor changes are
            // interfering with yours and you don't want to use SetMouseCursor() to change mouse cursor. You may want to
            // honor requests from imgui by reading GetMouseCursor() yourself instead.
...
enum ImGuiBackendFlags_ {
  ...
  ImGuiBackendFlags_HasSetMousePos = 1 << 2 // Back-end supports io.WantSetMousePos requests to reposition the OS mouse
...
enum ImGuiColorEditFlags_ {
  ...
  ImGuiColorEditFlags_NoSmallPreview = 1 << 4, //              // ColorEdit, ColorPicker: disable colored square preview
  ...
  ImGuiColorEditFlags_NoSidePreview = 1 << 8, //              // ColorPicker: disable bigger color preview on right side
  ...
  ImGuiColorEditFlags_RGB = 1 << 20, // [Inputs]     // ColorEdit: choose one among RGB/HSV/HEX. ColorPicker: choose any
                                     // combination using RGB/HSV/HEX.
...
struct ImGuiStyle {
  ...
  float FrameBorderSize;    // Thickness of border around frames. Generally set to 0.0f or 1.0f. (Other values are not well
...
struct ImGuiIO {
  ...
  ImGuiBackendFlags BackendFlags; // = 0                  // Set ImGuiBackendFlags_ enum. Set by imgui_impl_xxx files or
                                  // custom back-end to communicate features supported by the back-end.
  ...
  ImVec2 DisplayVisibleMin;       // <unset> (0.0f,0.0f)  // [obsolete] If you use DisplaySize as a virtual space larger than
                                  // your screen, set DisplayVisibleMin/Max to the visible area.
  ImVec2 DisplayVisibleMax;       // <unset> (0.0f,0.0f)  // [obsolete: just use io.DisplaySize] If the values are the same,
                                  // we defaults to Min=(0.0f) and Max=DisplaySize
  ...
  bool ConfigMacOSXBehaviors;        // = defined(__APPLE__) // OS X style: Text editing cursor movement using Alt instead of
                                     ...
                                     // Cmd+Arrows instead of Home/End, Double click selects by word instead of selecting whole
                                     // text, Multi-selection in lists uses Cmd/Super instead of Ctrl (was called
  ...
  bool ConfigResizeWindowsFromEdges; // = false          // [BETA] Enable resizing of windows from their edges and from
                                     // the lower-left corner. This requires (io.BackendFlags &
                                     // ImGuiBackendFlags_HasMouseCursors) because it needs mouse cursor feedback. (This
                                     // used to be the ImGuiWindowFlags_ResizeFromAnySide flag)
...
#else
  // This is only here to keep ImGuiIO the same size, so that IMGUI_DISABLE_OBSOLETE_FUNCTIONS can exceptionally be used
  // outside of imconfig.h.
  ...
  bool MouseDown[5];                    // Mouse buttons: 0=left, 1=right, 2=middle + extras. ImGui itself mostly only uses left button
                                        // (BeginPopupContext** are using right button). Others buttons allows us to track if the mouse is
                                        // being used by your application + available to user as a convenience via IsMouse** API.
  ...
  ImWchar InputCharacters[16 + 1];      // List of characters input (translated by user from keypress+keyboard state). Fill
                                        // using AddInputCharacter() helper.
  float NavInputs[ImGuiNavInput_COUNT]; // Gamepad inputs (keyboard keys will be auto-mapped and be written here by
  ...
  bool WantCaptureMouse;    // When io.WantCaptureMouse is true, imgui will use the mouse inputs, do not dispatch them to
                            // your main game/application (in both cases, always pass on mouse inputs to imgui). (e.g.
                            // unclicked mouse is hovering over an imgui window, widget is active, mouse was clicked over
                            // an imgui window, etc.).
  bool WantCaptureKeyboard; // When io.WantCaptureKeyboard is true, imgui will use the keyboard inputs, do not dispatch
                            // them to your main game/application (in both cases, always pass keyboard inputs to imgui).
                            // (e.g. InputText active, or an imgui window is focused and navigation is enabled, etc.).
  bool
    WantTextInput;              // Mobile/console: when io.WantTextInput is true, you may display an on-screen keyboard. This is set
                                // by ImGui when it wants textual keyboard input to happen (e.g. when a InputText widget is active).
  ...
  bool WantSaveIniSettings;     // When manual .ini load/save is active (io.IniFilename == NULL), this will be set to notify
                                // your application that you can call SaveIniSettingsToMemory() and save yourself.
...
IMGUI_API bool Begin(
  const char* name, bool* p_open, const ImVec2& size_on_first_use, float bg_alpha_override = -1.0f,
  ImGuiWindowFlags flags = 0); // Use SetNextWindowSize(size, ImGuiCond_FirstUseEver) + SetNextWindowBgAlpha() instead.
...
class ImVector
{
 public:
  ...
  // NB: It is forbidden to call push_back/push_front/insert with a reference pointing inside the ImVector data itself!
...
// used. The callback function should return 0 by default. Special processing:
// - ImGuiInputTextFlags_CallbackCharFilter:  return 1 if the character is not allowed. You may also set 'EventChar=0'
// as any character replacement are allowed.
// - ImGuiInputTextFlags_CallbackResize:      notified by InputText() when the string is resized. BufTextLen is set to
// the new desired string length so you can update the string size on your side of the fence. You can also replace Buf
// pointer if your underlying data is reallocated. No need to initialize new characters or zero-terminator as InputText
// will do it right after the resize callback.
struct ImGuiInputTextCallbackData {
  ...
  ImWchar EventChar;  // Character input                      // Read-write   // [CharFilter] Replace character or set to
                      // zero. return 1 is equivalent to setting EventChar=0;
  ...
  int BufSize;        // Buffer capacity in bytes             // Read-only    // [Resize,Completion,History,Always] Include
...
struct ImGuiListClipper {
  ...
  } // NB: Begin() initialize every fields (as we allow user to call Begin/End multiple times on a same instance if they
  // want).
...
enum ImDrawListFlags_ { ImDrawListFlags_AntiAliasedLines = 1 << 0,
...
struct ImDrawList {
  ...
  ImVector<ImDrawChannel> _Channels;     // [Internal] draw channels for columns API (not resized down so _ChannelsCount may
                                         // be smaller than _Channels.Size)
  ...
  IMGUI_API void AddImage(ImTextureID user_texture_id, const ImVec2& a, const ImVec2& b,
                          const ImVec2& uv_a = ImVec2(0, 0), const ImVec2& uv_b = ImVec2(1, 1), ImU32 col = 0xFFFFFFFF);
  ...
  IMGUI_API void AddBezierCurve(const ImVec2& pos0, const ImVec2& cp0, const ImVec2& cp1, const ImVec2& pos1, ImU32 col,
...
struct ImDrawData {
  ...
  ImVec2 DisplayPos;  // Upper-left position of the viewport to render (== upper-left of the orthogonal projection matrix
                      // to use)
  ...
  IMGUI_API void DeIndexAllBuffers(); // Helper to convert all buffers from indexed to non-indexed, in case you cannot
                                      // render indexed. Note: this is slow and most likely a waste of resources. Always
                                      // prefer indexed rendering!
  IMGUI_API void ScaleClipRects(
    const ImVec2& sc); // Helper to scale the ClipRect field of each ImDrawCmd. Use if your final output buffer is at a
                       // different scale than ImGui expects, or if there is a difference between your window resolution
...
struct ImFontAtlas {
  ...
  IMGUI_API ImFont* AddFontFromMemoryTTF(
    ...
    const ImWchar* glyph_ranges =
      NULL); // Note: Transfer ownership of 'ttf_data' to ImFontAtlas! Will be deleted after destruction of the atlas.
  ...
  // literal using the u8"Hello world" syntax. See FAQ for details. NB: Consider using GlyphRangesBuilder to build glyph
  ...
  // You can request arbitrary rectangles to be packed into the atlas, for your own purposes. After calling Build(), you
  // can query the rectangle position and render your pixels. You can also request your rectangles to be mapped as font
  // glyph (given a font + Unicode point), so you can render e.g. custom colorful icons and use them as regular glyphs.
  struct CustomRect {
    unsigned int ID;              // Input    // User ID. Use <0x10000 to map into a font glyph, >=0x10000 for other/internal/custom
                                  // texture data.
  ...
  int TexDesiredWidth;    // Texture width desired by user before Build(). Must be a power-of-two. If have many glyphs your
                          // graphics API have texture size restrictions you may want to increase texture width to decrease
                          // height.
...
struct ImFont {
  ...
  ImVector<float>
    IndexAdvanceX;                      //              // Sparse. Glyphs->AdvanceX in a directly indexable way (more cache-friendly, for
                                        //              CalcTextSize functions which are often bottleneck in large UI).
  ...
  int MetricsTotalSurface; //              // Total surface in pixels to get an idea of the font rasterization/texture
                           //              cost (not exact, we approximate the cost of padding between glyphs)
