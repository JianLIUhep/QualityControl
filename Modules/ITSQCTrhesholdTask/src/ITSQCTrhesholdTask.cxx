	...
	//	vector<Int_t> vHic;
		Int_t newHic=1;
                ...
                for (auto&& pixeldata : digits) {
                    ...
                    col = pixeldata.getColumn();
                    row = pixeldata.getRow();
		    if (ChipID!=ChipID_prev) gm->getChipId (ChipID, lay, sta, ssta, mod, chip);
		    //gm->getChipId (ChipID, lay, sta, ssta, mod, chip);
		   // cout<< " [OB AID] 1 "<<endl;
		...
		//----------------------------------------------------------------------------------------------------------
		   if (vHic.size()>0) {	
			...
			if (newHic==1) vHic.push_back(mod);
			
                   }
		   else vHic.push_back(mod);
	//--------------------------------------------------------------------------------------------------------------------
		    if (vLaySta.size()>0) {
			...
			//Do we have new chip?
		     if (vChip.size()>0) {
			    Int_t newChip=1;
                           ...
                           if (newChip==1)  vChip.push_back(chip);




                    }
                    else vChip.push_back(chip);
		//	 cout<< " [OB AID] 3 "<<endl;	
		...
		//	 cout<< " [OB AID] 3.5 "<< "sta = "<<sta<<" lay= "<<lay << " mod= "<<mod<<endl;
		    if(row >= 0 && col >= 0) hHitArray[sta][lay][mod][rowCS][colCS]++; //Need to check out, do we need -1?
...
*/
		if (FileFinish==1)
		{
			...
			for (int it = 0; it < vHic.size(); it++)  FillHistos(lay,sta,vHic[it]);
			 ...
			 //for (int it = 0; it < vLaySta.size(); it++)
			 {
						...
						*/
						...
						if (iLayer > 2){
							...
							cout<<"Check 1"<<endl;
							if (hDeadPixelSummary_OB_Stave_vs_Hic[iLayer] == NULL) cout<<"Problem with hDeadPixelSummary_OB_Stave_vs_Hic[iLayer]"<<endl;
							if ( hDeadPixelSummary_OB_Hic_vs_Chip[iLayer][iStave] == NULL) cout<<"Problem with  hDeadPixelSummary_OB_Hic_vs_Chip[iLayer][iStave]"<<endl;
 							if ( hDeadPixelSummary_OB_Hic_vs_Chip[iLayer][iStave]->ProjectionX("_D",iHic,iHic) == NULL) cout <<" Problem with hDeadPixelSummary_OB_Hic_vs_Chip[iLayer][iStave]->ProjectionX(,iHic,iHic)"<<endl;
							if (hTresholdSummary_OB_Stave_vs_Hic[iLayer] == NULL) cout<<"Problem with hTresholdSummary_OB_Stave_vs_Hic[iLayer]"<<endl;
							else cout<<"All is fine with hTresholdSummary_OB_Stave_vs_Hic[iLayer]"<<endl;
